...
def fetch_power_daily(lat,lon,start,end, params, retries=5):
    url=(
    ...
    for attempt in range(retries):
        ...
        if r.status_code == 200:
            ...
            wait = 5 * (attempt + 1)
            print(f"429 Too many requests. Waiting {wait}'s berfore retry...")
            time.sleep(wait)
        else:
            r.raise_for_status()
...
if __name__ == "__main__":
    ...
    for _, row in subset.iterrows():
        ...
        for y in years:
            ...
            print( " saved", outfile)
            time.sleep(4) # being gentle on the API